            ...
            <div className="bg-white rounded-[2.5rem] shadow-2xl max-w-2xl w-full overflow-hidden animate-in zoom-in duration-200">
                ...
                <div className="p-8 grid md:grid-cols-2 gap-8">
                    ...
                    <div className="space-y-6">
                        <div>
                            <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest mb-3 text-left">Shipping Info</p>
                            <div className="bg-slate-50 p-5 rounded-[2rem] space-y-3">
                                <div className="flex items-center gap-3 text-xs font-bold text-slate-700">
                                    <User size={14} className="text-blue-500"/> {order.CustomerName}
                                </div>
                                <div className="flex items-center gap-3 text-xs font-bold text-slate-700">
                                    <Phone size={14} className="text-blue-500"/> {order.CustomerPhone || 'N/A'}
                                </div>
                                <div className="flex items-start gap-3 text-xs font-bold text-slate-700 leading-tight">
                                    <MapPin size={14} className="text-blue-500 flex-shrink-0 mt-1"/> {order.CustomerAddress || 'N/A'}
                                </div>
                            </div>
                        </div>

                        {/* Logic hiển thị nút bấm khác nhau */}
                        <div className="space-y-2">
                            <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest mb-3 text-left italic">Available Actions</p>
                            
                            {/* NẾU LÀ ADMIN: Hiện nút chuyển trạng thái */}
                            {isAdmin ? (
                                <div className="space-y-2">
                                    {order.Status === 'Pending' && (
                                        <button onClick={() => onUpdateStatus(order.OrderID, 'Shipping')} className="w-full py-4 bg-blue-600 text-white rounded-2xl font-black text-[10px] uppercase flex items-center justify-center gap-2 hover:bg-blue-700 transition-all">
                                            <Truck size={16}/> Confirm & Ship
                                        </button>
                                    )}
                                    {order.Status === 'Shipping' && (
                                        <button onClick={() => onUpdateStatus(order.OrderID, 'Delivered')} className="w-full py-4 bg-emerald-600 text-white rounded-2xl font-black text-[10px] uppercase flex items-center justify-center gap-2 hover:bg-emerald-700 transition-all">
                                ...
                                /* NẾU LÀ KHÁCH: Chỉ hiện nút Hủy nếu đang Pending */
                                order.Status === 'Pending' ? (
                                    <button onClick={() => onUpdateStatus(order.OrderID, 'Cancelled')} className="w-full py-4 bg-red-50 text-red-600 rounded-2xl font-black text-[10px] uppercase border border-red-100 hover:bg-red-100 transition-all flex items-center justify-center gap-2">
                                        <XCircle size={16}/> Cancel My Order
                                    </button>
                                ) : (
                                    <div className="p-4 bg-slate-50 rounded-2xl text-center text-[10px] font-black text-slate-400 uppercase italic">
                    ...
                    <div>
                        <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest mb-3 text-left">Order Summary</p>
                        ...
                        <div className="mt-6 pt-4 border-t border-dashed border-slate-200 flex justify-between items-center">
                            ...
                            <span className="text-xl font-black text-blue-600 tracking-tighter">{Number(order.TotalPrice).toLocaleString()}đ</span>
                        </div>
                    </div>
                </div>
            </div>
